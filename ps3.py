...
class Room(object):
    ...
    def __init__(self, width, height, dust_amount):
        ...
        for i in range(self.width):
            for j in range(self.height):
                self.dust_dict[(i, j)] = dust_amount
    ...
    def get_dust_amount(self, w, h):
        ...
        Returns: a float
        """
        return float(self.dust_dict[(w, h)])
    ...
    def clean_tile_at_position(self, pos, cleaning_volume):
        ...
        """
        i = math.floor(pos.get_x())
        j = math.floor(pos.get_y())
        if self.dust_dict[(i,j)] - cleaning_volume > 0:
            self.dust_dict[(i,j)] = self.dust_dict[(i,j)] - cleaning_volume
        else: 
            self.dust_dict[(i,j)] = 0
    ...
    def get_random_position(self):
        ...
        # return Position(x,y)
        # finds random x and y values inside the room
        x = random.uniform(0,self.width)
        y = random.uniform(0, self.height)
        # returns a position object with these random values
        return Position(x, y)
...
class NormalRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        cleaning_volume = self.cleaning_volume
        position = self.get_position()
        room = self.room
        random_new_location = random.uniform(0,360)
        
        new_position = position.get_new_position(direction, speed)
        if not room.is_position_in_room(new_position):
            self.set_direction(random_new_location)
        else: 
            self.set_position(new_position)
            room.clean_tile_at_position(new_position, cleaning_volume)
...
class ClumsyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        drop_dust = self.does_drop_dust()
        dropped_dust = random.uniform(0, 0.5)
        if drop_dust:
            room.clean_tile_at_position(position, -(dropped_dust))
        new_position = position.get_new_position(direction, speed)
        if not room.is_position_in_room(new_position):
            self.set_direction(random_new_location)
        else: 
...
class SensingRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        dust = self.scan_surrounding_area()
        max_dust = 0
        for dust_amount in dust.values():
            if dust_amount > max_dust:
                max_dust = dust_amount 
        angle_list = []
        for angle in dust: 
            if dust[angle] == max_dust:
                angle_list.append(angle)
        random_angle = random.choice(angle_list)
        
        new_position = position.get_new_position(random_angle, speed)
        if not room.is_position_in_room(new_position):
            self.set_direction(random_new_location)
        else: 
            self.set_position(new_position)
...
def run_simulation(num_robots, speed, cleaning_volume, width, height, dust_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        for robot in range(num_robots):
            new_robot = robot_type(room, speed, cleaning_volume)
...
#       of a 20x20 room?
#sensing robot is fastest, followed by normal robot, then lastly clumsy robot. 
#
